...
def load_df(PATH):
    ...
    csv_path = os.path.join(PATH, 'Simulations','Data_processed','data_test_scaled.csv')
...
# Calcule des valeurs SHAP pour l'échantillon donné
    ...
    explainer_local = shap.Explainer(load_clf, X)
    shap_val_local = explainer_local(X)
    print("shap_val_local =", shap_val_local)
...
# Prédiction
    ...
    pred_proba = {
        ...
        'shap_values_local': {
        'values': shap_val_local.values.tolist(),
        'base_values': shap_val_local.base_values.tolist(),
        'data': shap_val_local.data.tolist(),
        'feature_names': shap_val_local.feature_names
        },
        'X_features': {
